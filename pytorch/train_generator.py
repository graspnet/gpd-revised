...
for test in range(1, 5):
    file_name = path + "train" + str(test) + "/labels_train" + str(test) + ".npy"
    ...
    for label in labels:
        if 0 < label < 0.11:
            positive_img.append(path + "train" + str(test) + "/image/" + str(img_cnt).zfill(6) + ".jpg")
        else:
            negative_img.append(path + "train" + str(test) + "/image/" + str(img_cnt).zfill(6) + ".jpg")
        img_cnt += 1
...
with open("../mydata/train.json", "w") as file:
    json.dump(f, file)
